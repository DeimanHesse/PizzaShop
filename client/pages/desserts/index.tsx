...
const Desserts: NextPage = ({hostname}) => {
    //useselector обращается к гидрированным данным
    const desserts: IProduct = useSelector((state) => state.products?.desserts);
    ...
    const sendProduct = async (e) => {
        ...
        dispatch(addOneDessert(res.data));
    ...
    const deleteProduct = async (item) => {
        ...
        dispatch(deleteDessert(item.id));
    };
...
export const getServerSideProps = wrapper.getServerSideProps(
    (store) => async (context) => {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_HOST}/products/desserts`);

        const desserts = {
            desserts: [...res.data],
        };
        await store.dispatch(addDesserts(desserts));
        // await store.dispatch(addDesserts(process.env.HOSTNAME));
